...
?>
    ...
    <header>
        ...
        <nav class="menu">
            ...
            <button class="active"
                    onclick="window.location.href='<?= $buildUrl('index.php', ['page' => 'coordinateur-etude']) ?>'">
    ...
    <main class="folders">
        ...
        <?php if ($action === 'view' && $studentData): ?>
            ...
            <form method="post" action="index.php?page=update_student&lang=<?= htmlspecialchars($lang) ?>"
                  enctype="multipart/form-data" class="creation-form">
                <div class="form-section">
                    ...
                    <input type="text" name="nom" value="<?= htmlspecialchars(strval($studentData['Nom'] ?? '')) ?>"
                           disabled class="input-disabled">

                    <label><?= $t(['fr' => 'Prénom', 'en' => 'First Name']) ?></label>
                    <input type="text" name="prenom"
                           value="<?= htmlspecialchars(strval($studentData['Prenom'] ?? '')) ?>" disabled
                           class="input-disabled">

                    <label><?= $t(['fr' => 'Né(e) le', 'en' => 'Date of Birth']) ?></label>
                    <input type="date" name="naissance"
                           value="<?= htmlspecialchars(strval($studentData['DateNaissance'] ?? '')) ?>" disabled
                           class="input-disabled">
                    ...
                    <input type="email" name="email_perso"
                           value="<?= htmlspecialchars(strval($studentData['EmailPersonnel'] ?? '')) ?>" disabled
                           class="input-disabled">

                    <label><?= $t(['fr' => 'Email AMU', 'en' => 'AMU Email']) ?></label>
                    <input type="email" name="email_amu"
                           value="<?= htmlspecialchars(strval($studentData['EmailAMU'] ?? '')) ?>" disabled
                           class="input-disabled">

                    <label><?= $t(['fr' => 'Téléphone', 'en' => 'Phone']) ?></label>
                    <input type="text" name="telephone"
                           value="<?= htmlspecialchars(strval($studentData['Telephone'] ?? '')) ?>" disabled
                           class="input-disabled">

                    <label><?= $t(['fr' => 'Code Département', 'en' => 'Department Code']) ?></label>
                    <input type="text" name="departement"
                           value="<?= htmlspecialchars(strval($studentData['CodeDepartement'] ?? '')) ?>" disabled
                           class="input-disabled">
                ...
                <div class="form-section documents-section">
                    ...
                    <div class="document-block">
                        ...
                        <?php if (!empty($pieces['photo'])): ?>
                            <div class="document-preview">
                                <img src="data:image/jpeg;base64,<?= strval($pieces['photo']) ?>" alt="Photo"
                                     class="photo-preview"><br>
                                <a href="data:image/jpeg;base64,<?= strval($pieces['photo']) ?>"
                                   download="photo_<?= $numEtu ?>.jpg" class="btn-download">
                    ...
                    <div class="document-block">
                        ...
                        <?php if (!empty($pieces['cv'])): ?>
                            <div class="document-preview">
                                <a href="data:application/pdf;base64,<?= strval($pieces['cv']) ?>"
                                   download="cv_<?= $numEtu ?>.pdf" class="btn-download">
                    ...
                    <div class="document-block">
                        ...
                        <?php if (!empty($pieces['lettre_motivation'])): ?>
                            <div class="document-preview">
                                <a href="data:application/pdf;base64,<?= strval($pieces['lettre_motivation']) ?>"
                                   download="lettre_<?= $numEtu ?>.pdf" class="btn-download">
                                    <?= $t(['fr' => 'Télécharger', 'en' => 'Download']) ?>
                                </a>
                            </div>
                        <?php else: ?>
            ...
            <div class="filters-container">
                ...
                <div class="filters">
                    ...
                    <?php if ($hasActiveFilters): ?>
                        <div class="filter-group">
                            <a href="<?= $buildUrl('index.php', ['page' => 'coordinateur-etude']) ?>" class="btn-reset">
            ...
            <table id="table-etudiants">
                ...
                <?php foreach ($paginatedData as $etudiant):
                    ...
                    <tr class="clickable-row"
                        ...
                        onclick="window.location.href='<?= $buildUrl('index.php', ['page' => 'coordinateur-etude', 'action' => 'view', 'numetu' => $numEtu]) ?>'">
...
$title = $t([
    'fr' => 'Coordinateur d\'étude - Relations Internationales AMU',
    'en' => 'Study Coordinator - International Relations AMU',
]);
...
$metaDescription = $t([
    'fr' => "Espace coordinateur d'étude — gestion des mobilités étudiantes.",
    'en' => 'Study coordinator space — student mobility management.',
]);
