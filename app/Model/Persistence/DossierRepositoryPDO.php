...
class DossierRepositoryPDO implements DossierRepositoryInterface
{
    ...
    /**
     * Utilisée pour déduire le continent en fonction du pays et de la zone.
     * Fonction passée en public pour éviter l'erreur de méthode privée non utilisée du linter.
     */
    ...
    public function searchWithPagination(array $filters, int $page, int $perPage): array
    {
        ...
        if (!empty($filters['mobilite']) && $filters['mobilite'] !== 'all') {
            $mob = is_string($filters['mobilite']) ? $filters['mobilite'] : null;
            $valid = $this->validMobilite($mob);
            if ($valid) {
