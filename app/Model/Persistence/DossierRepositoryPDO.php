...
class DossierRepositoryPDO implements DossierRepositoryInterface
{
    ...
    public function getDossierStats(): DossierStats
    {
        try {
            $stmt = $this->db->query("
                SELECT COUNT(*) as total, SUM(CASE WHEN IsComplete = 1 THEN 1 ELSE 0 END) as completed
                FROM dossiers
            ");
        ...
        try {
            $stmt = $this->db->query("
                SELECT NumEtu, Nom, Prenom, EmailPersonnel as email, Telephone, Type, Zone,
                       DateNaissance, Sexe, Adresse, CodePostal, Ville, EmailAMU, CodeDepartement, Pays,
                       Campus, Discipline, NiveauEtude, Formation, Moyenne, AvisDRI, 
                       DateDebut, Langues, MobiliteAnterieure,
        ...
        try {
            $stmt = $this->db->prepare("
                SELECT NumEtu, Nom, Prenom, DateNaissance, Sexe, Adresse, CodePostal, Ville,
                       EmailPersonnel, EmailAMU, Telephone, CodeDepartement, Type, Zone, Pays,
                       Campus, Discipline, NiveauEtude, Formation, Moyenne, AvisDRI,
                       DateDebut, Langues, MobiliteAnterieure,
                       IsComplete, PiecesJustificatives, status
    ...
    public function create(array $data): bool
    {
        try {
            ...
            return $stmt->execute([
                ...
                ':CodeDepartement' => $data['CodeDepartement'] ?? null, ':Type' => $data['Type'] ?? null,
                ':Zone' => $data['Zone'] ?? null, ':Pays' => $data['Pays'] ?? null,
                ':Campus' => $data['Campus'] ?? null, ':Discipline' => $data['Discipline'] ?? null,
                ':NiveauEtude' => $data['NiveauEtude'] ?? null, ':Formation' => $data['Formation'] ?? null,
                ':Moyenne' => $data['Moyenne'] ?? null, ':AvisDRI' => $data['AvisDRI'] ?? null,
                ':DateDebut' => $data['DateDebut'] ?? null, ':Langues' => $data['Langues'] ?? null,
        ...
        try {
            $stmt = $this->db->prepare("
                ...
                SET 
                    ...
                    Telephone = COALESCE(:Telephone, Telephone), CodeDepartement = COALESCE(:CodeDepartement, CodeDepartement), Type = COALESCE(:Type, Type),
                    ...
                    Formation = COALESCE(:Formation, Formation), Moyenne = COALESCE(:Moyenne, Moyenne), AvisDRI = COALESCE(:AvisDRI, AvisDRI),
                    DateDebut = COALESCE(:DateDebut, DateDebut), Langues = COALESCE(:Langues, Langues), MobiliteAnterieure = COALESCE(:MobiliteAnterieure, MobiliteAnterieure),
    ...
    public function searchWithPagination(array $filters, int $page, int $perPage): array
    {
        ...
        try {
            ...
            return ['data' => $stmt->fetchAll(PDO::FETCH_ASSOC), 'total' => $totalCount, 'totalPages' => $totalCount > 0 ? (int)ceil($totalCount / $perPage) : 0];
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                ON DUPLICATE KEY UPDATE 
                    ...
                    Telephone = COALESCE(VALUES(Telephone), dossiers.Telephone), CodeDepartement = COALESCE(VALUES(CodeDepartement), dossiers.CodeDepartement),
                    ...
                    Formation = COALESCE(VALUES(Formation), dossiers.Formation), Moyenne = COALESCE(VALUES(Moyenne), dossiers.Moyenne),
                    AvisDRI = COALESCE(VALUES(AvisDRI), dossiers.AvisDRI), DateDebut = COALESCE(VALUES(DateDebut), dossiers.DateDebut),
                    Langues = COALESCE(VALUES(Langues), dossiers.Langues), MobiliteAnterieure = COALESCE(VALUES(MobiliteAnterieure), dossiers.MobiliteAnterieure)
            ");
