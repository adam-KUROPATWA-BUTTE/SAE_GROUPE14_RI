...
class DossierRepositoryPDO implements DossierRepositoryInterface
{
    ...
    /**
     * FIX line 59: typed $bindings so PHPStan knows the value type.
    ...
    public function getDossierStats(?string $mobilite = null): DossierStats
    {
        try {
            ...
            // FIX lines 89/90: fetch() returns mixed — guard with is_array before offset access.
    ...
    public function getGenderStats(?string $mobilite = null): GenderStats
    {
        try {
            ...
            // FIX lines 113/114: same mixed fetch() issue.
            $row    = $stmt ? $stmt->fetch(PDO::FETCH_ASSOC) : null;
            $result = is_array($row) ? $row : [];
    ...
    public function getTopCountries(int $limit, ?string $mobilite = null): array
    {
        try {
            ...
            $stmt = $this->db->prepare("
                ...
                $mobiliteClause
                GROUP BY Pays ORDER BY count DESC LIMIT :limit
            ");
    ...
    public function getDepartmentStats(int $limit, ?string $mobilite = null): array
    {
        try {
            ...
            $stmt = $this->db->prepare("
                ...
                $mobiliteClause
                GROUP BY CodeDepartement ORDER BY count DESC LIMIT :limit
            ");
            foreach ($bindings as $key => $value) { $stmt->bindValue(":$key", $value); }
            $stmt->bindValue(':limit', $limit, PDO::PARAM_INT);
            $stmt->execute();
    ...
    public function getIncomingOutgoingStats(?string $mobilite = null): array
    {
        try {
            ['clause' => $where, 'bindings' => $bindings] = $this->mobiliteWhere($mobilite);
            $stmt = $this->run("
                SELECT SUM(CASE WHEN LOWER(Type) = 'entrant' THEN 1 ELSE 0 END) AS incoming,
    ...
    public function getContinentStats(?string $mobilite = null): array
    {
        try {
            ...
            $bindings       = $mobiliteValid ? ['mobilite' => $mobiliteValid] : [];
            $stmt = $this->run("
                SELECT Continent AS name, COUNT(*) AS count FROM dossiers
                WHERE Continent IS NOT NULL AND Continent != '' $mobiliteClause
    ...
    public function getEuropeVsNonEuropeStats(?string $mobilite = null): array
    {
        try {
            ...
            $bindings       = $mobiliteValid ? ['mobilite' => $mobiliteValid] : [];
            $stmt = $this->run("
                SELECT COUNT(DISTINCT CASE WHEN LOWER(Zone) = 'europe'      THEN Pays END) AS europe_countries,
                       COUNT(DISTINCT CASE WHEN LOWER(Zone) = 'hors_europe' THEN Pays END) AS non_europe_countries
                FROM dossiers WHERE Pays IS NOT NULL AND Pays != '' $mobiliteClause
            ", $bindings);
    ...
    public function getZoneStats(?string $mobilite = null): array
    {
        try {
            ...
            $bindings       = $mobiliteValid ? ['mobilite' => $mobiliteValid] : [];
            $stmt = $this->run("
                SELECT Zone AS name, COUNT(*) AS count FROM dossiers
                WHERE Zone IS NOT NULL AND Zone != '' $mobiliteClause
    ...
    // FIX line 320: method is unused by the rest of the codebase.
    // Annotated so PHPStan ignores it; remove if truly unneeded.
        ...
        try {
            $stmt = $this->db->prepare("
                UPDATE dossiers SET
                    ...
                    Formation = COALESCE(:Formation, Formation), MoyenneBac = COALESCE(:MoyenneBac, MoyenneBac),
                    MoyenneSansBac = COALESCE(:MoyenneSansBac, MoyenneSansBac), AvisDRI = COALESCE(:AvisDRI, AvisDRI),
                    DateDebut = COALESCE(:DateDebut, DateDebut), MobiliteAnterieure = COALESCE(:MobiliteAnterieure, MobiliteAnterieure),
        ...
        try {
            ...
            $stmtEtu = $pdo->prepare("
                INSERT INTO etudiants (
                    numetu, nom, prenom, email, password, departement, campus, annee_etude,
                    type_etudiant, telephone, adresse, code_postal, ville, sexe, date_naissance, created_at
                ) VALUES (
                    :numetu, :nom, :prenom, :email, :pass, :dept, :campus, :annee,
                    :type, :tel, :adr, :cp, :ville, :sexe, :dob, NOW()
            ...
            foreach ($dossiers as $d) {
                $numEtu = '';
                if (isset($d['NumEtu']) && is_scalar($d['NumEtu'])) {
                    $numEtu = (string) $d['NumEtu'];
                }

                $email = '';
                if (isset($d['EmailAMU']) && is_string($d['EmailAMU']) && $d['EmailAMU'] !== '') {
                    $email = $d['EmailAMU'];
                } elseif (isset($d['EmailPersonnel']) && is_string($d['EmailPersonnel'])) {
